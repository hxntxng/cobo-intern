...
def a_deposit(w3, acct, val):
    ...
    approve_cbeth_token_txn = cbeth_token_contract.functions.approve(uniswap_router_address, unlim).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
    ...
    deadline = 1695788793
    addLiquidity_uniswap_txn = uniswap_router_contract.functions.addLiquidityETH(cbeth_token_address, token_amt, token_min, eth_min, acct_address, deadline).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
    ...
    approve_uniswap_txn = uniswap_pair_contract.functions.approve(based_distributor_address, val).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
...
def a_get_val(w3, acct):
    ...
    w3.eth.send_raw_transaction(deposit_distributor_signtxn.rawTransaction)
    breakpoint()
    val_txn = based_distributor_contract.functions.userInfo(6, acct.address).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
