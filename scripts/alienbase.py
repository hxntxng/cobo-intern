...
def deposit(acct, val):
    ...
    approve_cbeth_token_txn = cbeth_token_contract.functions.approve(uniswap_router_address, unlim).build_transaction({'chainId': 8453, 'gas': 200000, 'gasPrice': w3.to_wei('0.01', 'gwei'), 'nonce': nonce,})
    approve_cbeth_token_signtxn = w3.eth.account.sign_transaction(approve_cbeth_token_txn, private_key = acct._private_key)
    # w3.eth.send_raw_transaction(approve_cbeth_token_signtxt.rawTransaction)
    ...
    deadline = 1695788793
    addLiquidity_uniswap_txn = uniswap_router_contract.functions.addLiquidityETH(cbeth_token_address, token_amt, token_min, eth_min, acct_address, deadline)
    addLiquidity_uniswap_signtxn = w3.eth.account.sign_transaction(addLiquidity_uniswap_txn, private_key = acct._private_key)
    ...
    approve_uniswap_txn = uniswap_pair_contract.functions.approve(based_distributor_address, val)
    approve_uniswap_signtxn = w3.eth.account.sign_transaction(approve_uniswap_txn, private_key = acct._private_key)
    ...
    deposit_distributor_txn = based_distributor_contract.functions.deposit(pid, val)
    deposit_distributor_signtxn = w3.eth.account.sign_transaction(deposit_distributor_txn, private_key = acct._private_key)
...
def withdraw(acct):
    ...
    deposit_distributor_signtxn = w3.eth.account.sign_transaction(deposit_distributor_txn, private_key = acct._private_key)
    # w3.eth.send_raw_transaction(deposit_distributor_signtxn.rawTransaction)
    val = based_distributor_contract_contract.functions.userInfo(6, acct).call()[0]
    withdraw_distributor_txn = based_distributor_contract.functions.withdraw(6, val)   
    withdraw_distributor_signtxn = w3.eth.account.sign_transaction(withdraw_distributor_txn, private_key = acct._private_key)
