...
def c_deposit(w3, acct, val):
    ...
    allowed = True
    allow_proxy_txn = proxy_contract.functions.allow(base_bulk_address, allowed).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
    ...
    invoke_bulker_txn = base_bulk_contract.functions.invoke(deposit_action, data).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
...
def c_withdraw(w3, acct):
    ...
    invoke_bulker_txn = base_bulk_contract.functions.invokeTransaction(withdraw_action, data).build_transaction({'chainId': chainId, 'gas': 200000, 'gasPrice': w3.toWei('0.01', 'gwei'), 'nonce': nonce,})
    invoke_bulker_signtxn = w3.eth.account.sign_transaction(invoke_bulker_txn, private_key = acct._private_key)
    w3.eth.send_raw_transaction(invoke_bulker_signtxn.rawTransaction)
    return